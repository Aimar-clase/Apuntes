...
window.App.Game = (function() {
    ...
    const GameInstance = {
        ...
        start() {
            ...
            this.loop(0);
            this.scheduleNextSpawn(); // Start the recursive spawn loop
        ...
        scheduleNextSpawn() {
            ...
            // Recursive setTimeout pattern for variable spawn rate
        ...
        loop(timestamp) {
            ...
            // Removed spawn check from here, handled by scheduleNextSpawn
        ...
        spawnWord() {
            ...
            // Parse initial X position
            const initialX = parseFloat(wordEl.style.left) || 0;
        ...
        updateWords() {
            ...
            for (let i = this.state.words.length - 1; i >= 0; i--) {
                ...
                // Sine wave movement
                // x = initialX + amplitude * sin(y * frequency)
                const amplitude = 50;
                const frequency = 0.02;
                word.x = word.initialX + Math.sin(word.y * frequency) * amplitude;

                // Boundary checks to keep word inside screen
                const maxLeft = this.dom.gameArea.clientWidth - 120;
                if (word.x < 0) word.x = 0;
                if (word.x > maxLeft) word.x = maxLeft;
        ...
        gameOver() {
            ...
            clearTimeout(this.state.spawnTimer);
